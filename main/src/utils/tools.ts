 ...
 */
// @ts-ignore
export const recursionTree = (tree: any[]) => {
...
/**
 * 将/middle-school：3和/high-school：4的地址统一带上参数
 * @param url URL地址
 */
export const getParamUrl = (url: string) => {
	let str = url;
	if (url.indexOf('/middle-school') > 0) {
		str += '?stage=3';
	} else if (url.indexOf('/high-school') > 0) {
		str += '?stage=4';
	}
	return str;
};
